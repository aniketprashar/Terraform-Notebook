...
resource "azurerm_network_security_rule" "fe-rg" {
  ...
  source_address_prefix       = "${azurerm_network_interface.jbox-rg.private_ip_address}/32" #concatination
  destination_address_prefix  = "${azurerm_network_interface.be-rg.private_ip_address}/32"   #concatination
  resource_group_name         = azurerm_resource_group.be-rg.name
